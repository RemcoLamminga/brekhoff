<?php
/**
 * Content Tekst Afbeelding — Render Template
 */

// Block name prefix
$block_name = 'content-tekst-afbeelding';
...
$layout_class = $map[$switch] ?? '';


$margin_class = (!empty($eyebrow) || !empty($title)) ? '' : $block_name . '--margin-0';

$stijl = get_field($block_name . '_section-bg');
...
if (!empty($block['data']['_is_preview'])) {
	...
	<figure style="margin: 0; width: 100%; height: auto;">
		<img src="<?php echo esc_url($url . '/preview.jpg'); ?>"
			alt="<?php echo esc_attr('Content Tekst Afbeelding block preview'); ?>" style="width: 100%; height: auto;" />
	</figure>
	<?php
	return;
}
		...
		<div class="<?php echo esc_attr($block_name); ?>__inner <?php echo $layout_class; ?>">
			...
			<div class="<?php echo esc_attr($block_name); ?>__content">
				<div class="<?php echo esc_attr($block_name); ?>__content-inner">
					...
					<?php if ($title): ?>
						<h2 class="<?php echo esc_attr($block_name); ?>__title <?php echo esc_attr($margin_class); ?>">
							<?php echo esc_html($title); ?></h2>
					<?php endif; ?>

					<?php if ($content): ?>
						<div class="<?php echo esc_attr($block_name); ?>__text <?php echo esc_attr($margin_class); ?>">
